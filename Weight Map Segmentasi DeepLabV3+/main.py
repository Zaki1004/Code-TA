...
import numpy as np
import cv2, glob, os, json, matplotlib.pyplot as plt
...
MODEL_PATH = "/content/drive/MyDrive/New/E-150-DeepLabV3+/deeplabv3+_best_ufl.keras"
IMAGE_DIR = "/content/drive/MyDrive/New/AllDataImagesCleaning/"
SAVE_DIR  = "/content/drive/MyDrive/New/E-150-DeepLabV3+/HasilWeightMap(25%)/"
...
if not image_paths:
    ...
    for path in tqdm(image_paths, desc="🔄 Memproses gambar", unit="img"):
        ...
        if pct_damage > DETECTION_THRESHOLD:
            ...
            tqdm.write(f"{fname} rusak {pct_damage:.2f}% → disalin ke HasilTerbaik/")
        else:

         if (len(all_metadata) % 100) == 0:
            tqdm.write(f"{len(all_metadata)}/{len(image_paths)} gambar diproses...")
